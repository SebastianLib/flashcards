...
export const createNewSet = createAsyncThunk(
  "flashcards/createNewSet",
  async ({ title, description, items, userRef }) => {
    try {
  ...
  async (currentUser) => {
    try {
      const axiosResponse = await axios.get(
        "http://localhost:3000/flashcards/getFlashcards",
        {
          params: {
...
export const updateSet = createAsyncThunk(
  ...
  async (flashcards) => {
    try {
      const axiosResponse = await axios.post(
        "http://localhost:3000/flashcards/updateSet",
        {
...
export const removeSet = createAsyncThunk(
  "flashcards/removeSet",
  async ({ setId, userRef }) => {
    try {
      const axiosResponse = await axios.delete(
        "http://localhost:3000/flashcards/removeSet",
        {
          data: {
            setId,
            userRef,
...
export const removeFlashcard = createAsyncThunk(
  ...
  async ({ setId, flashcardId, userRef }) => {
    try {
      const axiosResponse = await axios.delete(
        "http://localhost:3000/flashcards/removeFlashcard",
        {
          data: {
            setId,
            flashcardId,
...
export const addNewFlashcard = createAsyncThunk(
  ...
  async ({ concept, definition, _id, userRef }) => {
    try {
      const axiosResponse = await axios.post(
        "http://localhost:3000/flashcards/addNewFlashcard",
        {
          concept,
          definition,
          _id,
